...
//			simulate_key(SDLK_DOWN, STATE_TYPED);
			...
			combo_help |= 1;
			if (combo_help == 3) show_help = 1;
			else simulate_key(SDLK_LEFT, STATE_TYPED);
		} else if(event->key.keysym.sym == KEY_ARROW_RIGHT) {
			combo_help |= 2;
			if (combo_help == 3) show_help = 1;
			else simulate_key(SDLK_RIGHT, STATE_TYPED);
#else
...
#ifdef MIYOOMINI
		if(event->key.keysym.sym == KEY_ARROW_LEFT) {
			combo_help &= 2;
		} else if(event->key.keysym.sym == KEY_ARROW_RIGHT) {
			combo_help &= 1;
		} else
